"""
Following code adds the function 'condsets_for_depscore' to the PCMCI class from Tigramite (author: Jakob Runge).
Needed for computing the dependency score for aggregation validity.
"""
...
class PCMCI(PCMCIbase):
    ...
    * false discovery control and confidence interval estimation
    ...
    def condsets_for_depscore(self,
                           mode,
                           pc_alpha,
                           tau_max,
                           tau_min=0,
            ...
            """Computes the dependence score for aggregation consistency.
            ...
            sepsets : dictionary
                Separating sets. See paper for details.
            condsets : dictionary
                Conditioning set per pair of variable that yielded the minimum p-val
                during skeleton phase. Needed for Depedence score.
            """
            N = self.N
            ...
            # independent
            sepsets = self._get_sepsets(tau_min, tau_max)
            condsets = self._get_sepsets(tau_min, tau_max)
                ...
                for ir, (i, j, abstau) in enumerate(remaining_pairs):
                    ...
                    if graph[i, j, abstau] != "":
                        ...
                        for q, S in enumerate(conditions):
                            ...
                            # Store minimum absolute test statistic value for sorting adjt (only internally used)
                            val_min[j][(i, -abstau)] = min(np.abs(val),
                                                           val_min[j].get(
                                                               (i, -abstau)))

                            # Save the conditioning set corresponding to maximum test statistic value
                            # print("\n=====\n Will Condsets Update? \n======\n")
                            if np.abs(val) >val_max[j].get((i, -abstau)):

                                # print("\n=====\n Condsets Updating with S: \n======\n",list(S))

                                if abstau == 0:
                                    condsets[((i, 0), j)] = condsets[
                                        ((j, 0), i)] = list(S)
                                else:
                                    condsets[((i, -abstau), j)] = list(S)

                            # Store maximum absolute test statistic value (for dependence score)
                            val_max[j][(i, -abstau)] = max(np.abs(val),
                                                           val_max[j].get(
                                                               (i, -abstau)))
                            ...
                            pval_max[j][(i, -abstau)] = max(pval,
                                                            pval_max[j].get(
                                                                (i, -abstau)))
                            # Store minimum p-value (for dependence score_
                            pval_min[j][(i, -abstau)] = min(pval,
                                                            pval_min[j].get(
                                                                (i, -abstau)))
...
# def compute_dep_score(data, graph_agg, condsets, d_macro, d_micro, pc_alpha, ci_test='parcorr_gcm_gmb'):
...
#     for (i, j) in itertools.product(range(d_macro), range(d_macro)):
#         for abstau in range(tau_min, tau_max + 1):
